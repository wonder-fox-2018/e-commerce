...
module.exports = {
    ...
    promote: function (req, res) {
        ...
        .then(user => {
            if (user) {
                if (user.isAdmin === 1) {
                    res.status(500).json({message: `${user.name} (${req.body.email}) is registered as an admin, there's no need for a promotion.`})
                } else {
                    User.updateOne({
                        email: req.body.email
                    }, {
                    ...
                    .then(() => {
                        res.status(200).json({message: `${user.name} (${req.body.email}) has been successfully promoted.`})
                    })
                    .catch(err => {
                        res.status(500).json({message: err})
                    })
                ...
                res.status(500).json({message: "Sorry, but we couldn't find anyone registered with that email."})
            }
        })
        .catch(err => {
            res.status(500).json({message: err})
